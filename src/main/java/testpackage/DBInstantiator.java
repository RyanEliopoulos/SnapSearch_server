...
public class DBInstantiator {
  // For establishing the db needed by the server program before running.
  // THIS STEP MUST BE DONE MANUALLY.  The daemon expects the db to already exist.
  ...
  private void insrtFile() {
    ...
    String sql =
        "INSERT INTO photos (userid, filedata, longitude, latitude)" + "VALUES (1, ?, ?, ?)";
  ...
  private void tutorialVersion(String dbname) {
    ...
    try {
      ...
      sql = "INSERT INTO users (name)" + "VALUES ('TANJA'), ('MIGUEL')";
